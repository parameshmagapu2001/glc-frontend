...
function RoadAproachForm({ farmlandId, documentIndex, documentDetails, onNext }: Props) {
  ...
  const handleStyleRadioChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setRoadType((event.target as HTMLInputElement).value);
  };

  const onSave = async () => {
    if (roadType === '0') {
      enqueueSnackbar('Please select the type of road', {
        variant: 'error',
        anchorOrigin: { vertical: 'bottom', horizontal: 'center' }
      });
      return;
    }
    if ((roadType === 'Private' || roadType === 'Government') && !widthOfRoad) {
      enqueueSnackbar('Please enter the width of road', {
        variant: 'error',
        anchorOrigin: { vertical: 'bottom', horizontal: 'center' }
      });
      return;
  ...
  const refreshDocumentData = useCallback(async () => {
    ...
    try {
      ...
      setRoadType(res.data.documentDetails?.roadType || '');
      setActivityTimeLine(res.data.activityTimeline);
      setTimelineViewStatus(res.data.timeLineView || false);
      setActiveView(res.data.timeLineView ? 'timeline' : 'file');
    } catch (err) {
      ...
      {activeView === 'file' &&
        ...
        >
          <Stack direction='row' justifyContent='space-between'>
            <Box width="40%">
              <Typography fontWeight="medium" sx={{ mb: 2 }}>Type of Road Aproach</Typography>
              ...
              <FormProvider {...methods}>
                <Stack sx={{ mb: 1 }}>
                  ...
                  >
                    <FormControlLabel value="Private" control={<Radio />} label="Private Road" />
                    <FormControlLabel value="Government" control={<Radio />} label="Government Road" />
                  </RadioGroup>


                  {(roadType === 'Government' || roadType === 'Private') &&
                    <Stack>
                      ...
                      <RHFTextField
                        ...
                        label=" Width of Road"
                        value={widthOfRoad}
                        onChange={(e) => setWidthOfRoad(e.target.value)}
...
export default RoadAproachForm;
