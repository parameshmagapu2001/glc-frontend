...
const activities = [
  {
    dateTime: 'Jan 14 • 8:00 PM',
    description: 'Lorem Ipsum has been the industry’s standard dummy text ever since the 1500s, consectetur adipiscing elit, sed do eiusmod. Lorem ipsum dolor sit amet, consectetur adipiscing elit.',
    files: ['File_name.pdf', 'File_name_1.pdf'],
    issueComment: 'Lorem Ipsum has been the industry’s standard dummy text ever since the 1500s, consectetur adipiscing elit, sed do eiusmod. Lorem ipsum dolor sit amet, consectetur adipiscing elit.do eiusmod',
    officer: 'Legality Officer'
  }
];

function RailwayTrackConnectivityForm({ farmlandId, documentIndex, documentDetails, onNext }: Props) {
  ...
  const [railwayConnectivity, setRailwayConnectivity] = useState('');
  ...
  useEffect(() => {
    getRailwayIdentifiers('Railway_Track_Distance');
    setComments(documentDetails.documentDetails?.comments || '');
    setRailwayConnectivity(documentDetails.documentDetails?.railwayConnectivity || '');
    setTrackDistance(documentDetails.documentDetails?.label || '0');
  ...
  const getRailwayIdentifiers = async (type: string) => {
  ...
  const handleStyleRadioChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setRailwayConnectivity((event.target as HTMLInputElement).value);
  };

  const onSave = async () => {
    if(!railwayConnectivity) {
      enqueueSnackbar('Please select railway connectivity', {
        variant: 'error',
    ...
    if (railwayConnectivity === 'Available' && trackDistance === '0') {
      enqueueSnackbar('Please select distance', {
        variant: 'error',
        anchorOrigin: { vertical: 'bottom', horizontal: 'center' }
      });
      return;
    ...
    const body: FarmlandDocumentBody = {
      ...
      documentDetails: {
        comments,
        railwayConnectivity,
        id: identifiers.find((item) => item.label === trackDistance)?.id,
        label: trackDistance,
  ...
  const refreshDocumentData = useCallback(async () => {
    ...
    try {
      ...
      setRailwayConnectivity(res.data.documentDetails?.railwayConnectivity || '');
      setTrackDistance(res.data.documentDetails?.label || '0');
      setActivityTimeLine(res.data.activityTimeline);
      setTimelineViewStatus(res.data.timeLineView || false);
      setActiveView(res.data.timeLineView ? 'timeline' : 'file');
    } catch (err) {
  ...
  const handleDistanceDropdownChange = (event: React.ChangeEvent<{ value: unknown }>) => {
    setTrackDistance(event.target.value as string);
  };
      ...
      {activeView === 'file' &&
        ...
        >
          <Stack direction='row' justifyContent='space-between' alignItems='center'>
            <Box width="40%">
              <Typography fontWeight="medium">Any Railway Track Connectivity</Typography>

              <FormProvider {...methods}>
                <Stack sx={{ mb: 1 }}>

                  <RadioGroup aria-label="text" name="video_style" value={railwayConnectivity}
...
export default RailwayTrackConnectivityForm;
