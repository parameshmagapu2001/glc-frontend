...
function ExistingTreesForm({ farmlandId, documentIndex, documentDetails, onNext }: Props) {
  ...
  useEffect(() => {
    ...
    setExistingTrees(documentDetails.documentDetails?.existingTrees || '0');
  ...
  const handleStyleRadioChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setExistingTrees((event.target as HTMLInputElement).value);
  };
  ...
  const onSave = async () => {
    if (existingTrees === '0') {
      enqueueSnackbar('Please select existing trees', {
        variant: 'error',
        anchorOrigin: { vertical: 'bottom', horizontal: 'center' }
      });
      return;
    }
    if (existingTrees === 'Available' && treesCount === '0') {
    ...
    const body: FarmlandDocumentBody = {
      ...
      documentDetails: {
        ...
        existingTrees,
        treesCount,
        treesCountId: identifiers.find((item) => item.label === treesCount)?.id,
      },
    };
  ...
  const refreshDocumentData = useCallback(async () => {
    ...
    try {
      ...
      setExistingTrees(res.data.documentDetails?.existingTrees || '0');
      ...
      {activeView === 'file' &&
        ...
        >
          <Stack direction='row' justifyContent='space-between'>
            <Box width="40%">
              <Typography fontWeight="medium">Any Existing Trees available surrounding land?</Typography>

              <FormProvider {...methods}>
                <Stack sx={{ mb: 1 }}>

                  <RadioGroup aria-label="text" name="video_style" value={existingTrees}
                  ...
                  <Stack spacing={3}>
                    {existingTrees === 'Available' &&
                      <Stack>
                        <Typography fontWeight="medium" sx={{ my: 2 }}>Trees Count</Typography>
                        <RHFSelect
                          name="trees"
...
export default ExistingTreesForm;
