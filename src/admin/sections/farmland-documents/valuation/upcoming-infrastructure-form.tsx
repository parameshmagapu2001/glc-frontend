...
function UpcomingInfrastructureForm({ farmlandId, documentIndex, documentDetails, onNext }: Props) {
  ...
  useEffect(() => {
    getInfrastructures('Infrastructures');
    setComments(documentDetails.documentDetails?.comments || '');
    setInfrastructure(documentDetails.documentDetails?.label || '0');
  ...
  const getInfrastructures = async (type: string) => {
  ...
  const onSave = async () => {
    if (infrastructure === '0') {
      enqueueSnackbar('Please select the infrastructure', {
        variant: 'error',
    ...
    const body: FarmlandDocumentBody = {
      ...
      documentDetails: {
        comments,
        id: identifiers.find((item) => item.label === infrastructure)?.id,
  ...
  const refreshDocumentData = useCallback(async () => {
    ...
    try {
      ...
      setInfrastructure(res.data.documentDetails?.label || '0');
      setActivityTimeLine(res.data.activityTimeline);
      setTimelineViewStatus(res.data.timeLineView || false);
      setActiveView(res.data.timeLineView ? 'timeline' : 'file');
    } catch (err) {
  ...
  const handleInfraDropdownChange = (event: React.ChangeEvent<{ value: unknown }>) => {
    setInfrastructure(event.target.value as string);
  };
          ...
          <Stack direction='row' spacing={5} justifyContent='space-between'>
            <Box width="40%">
              <FormProvider {...methods}>
                <Stack spacing={3}>
                  <Stack>
                    <Typography fontWeight="medium" sx={{ my: 2 }}>Upcoming Infrastructures in Future</Typography>
                    <RHFSelect
                      name="select"
                      placeholder="Select"
                      ...
                      onChange={handleInfraDropdownChange}
                      sx={{
                        width: 1
                      }}
                    >
...
export default UpcomingInfrastructureForm;
