...
function GeologicalAdvantagesForm({ farmlandId, documentIndex, documentDetails, onNext }: Props) {
  ...
  useEffect(() => {
    ...
    setAdvantages(documentDetails.documentDetails?.advantages || '');
  ...
  const handleStyleRadioChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setAdvantages((event.target as HTMLInputElement).value);
  };

  const onSave = async () => {
    if(advantages === '') {
      enqueueSnackbar('Please select the advantages', {
        variant: 'error',
  ...
  const refreshDocumentData = useCallback(async () => {
    ...
    try {
      ...
      setAdvantages(res.data.documentDetails?.advantages || '');
      setActivityTimeLine(res.data.activityTimeline);
      setTimelineViewStatus(res.data.timeLineView || false);
      setActiveView(res.data.timeLineView ? 'timeline' : 'file');
    } catch (err) {
      ...
      {activeView === 'file' &&
        ...
        >
          <Stack direction='row' justifyContent='space-between' alignItems='center' sx={{ mb: 2 }}>
            <Box width="40%">
              <Typography fontWeight="medium">Any Surrounding Mines & Geological Advantages?</Typography>

              <FormProvider {...methods}>
                <Stack sx={{ mb: 1 }}>

                  <RadioGroup aria-label="text" name="video_style" value={advantages}
...
export default GeologicalAdvantagesForm;
