...
function RecentTransactionForm({ farmlandId, documentIndex, documentDetails, onNext }: Props) {
  ...
  const [valuationPerAcre, setValuationPerAcre] = useState('');

  const [localMarketAcrePrice, setLocalMarketAcrePrice] = useState('');
  ...
  useEffect(() => {
    ...
    setTransactionType(documentDetails.documentDetails?.transactionType || '');
    setValuationPerAcre(documentDetails.documentDetails?.valuationPerAcre || '');
    setLocalMarketAcrePrice(documentDetails.documentDetails?.localMarketAcrePrice || '');
  ...
  const onSave = async () => {
    if (!transactionType) {
      enqueueSnackbar('Please select transaction type', {
        variant: 'error',
        anchorOrigin: { vertical: 'bottom', horizontal: 'center' }
      });
      return;
    }
    if (transactionType === 'Available' && !valuationPerAcre) {
      enqueueSnackbar('Please enter valuation per acre', {
        variant: 'error',
        anchorOrigin: { vertical: 'bottom', horizontal: 'center' }
      });
      return;
    ...
    if (transactionType === 'Available' && !localMarketAcrePrice) {

      enqueueSnackbar('Please enter local market acre price', {
        variant: 'error',
        anchorOrigin: { vertical: 'bottom', horizontal: 'center' }
      });
      return;
  ...
  const refreshDocumentData = useCallback(async () => {
    ...
    try {
      ...
      setTransactionType(res.data.documentDetails?.transactionType || '');
      setValuationPerAcre(res.data.documentDetails?.valuationPerAcre || '');
      setLocalMarketAcrePrice(res.data.documentDetails?.localMarketAcrePrice || '');
      setActivityTimeLine(res.data.activityTimeline);
      setTimelineViewStatus(res.data.timeLineView || false);
      setActiveView(res.data.timeLineView ? 'timeline' : 'file');
    } catch (err) {
      ...
      {activeView === 'file' &&
        ...
        >
          <Stack direction='row' justifyContent='space-between'>
            <Box width="40%">
              <Typography fontWeight="medium">Any recent transaction in surrounding lands?</Typography>

              <FormProvider {...methods}>
                <Stack sx={{ mb: 1 }}>

                  <RadioGroup aria-label="text" name="video_style" value={transactionType}
                ...
                <Stack spacing={3}>
                  {transactionType === 'Available' &&
                    <Stack>
                      <Typography fontWeight="medium" sx={{ my: 2 }}>Valuation Per Acre</Typography>
                      <RHFTextField
                        name="valuationPerAcre"
                        label=" Valuation Per Acre"
                        value={valuationPerAcre}
                        onChange={(e) => {
                          const input = e.target.value;
                          const numbersOnly = input.replace(/[^0-9]/g, '');
                          setValuationPerAcre(numbersOnly);
                        }}
                        placeholder="Enter Valuation Per Acre"
                  ...
                  {transactionType === 'Available' &&
                    <Stack>
                      <Typography fontWeight="medium" sx={{ my: 2 }}>Local Market Acre Price</Typography>
                      <RHFTextField
                        name="localMarketAcrePrice"
                        label=" Local Market Acre Price"
                        value={localMarketAcrePrice}
                        onChange={(e) => {
                          const input = e.target.value;
                          const numbersOnly = input.replace(/[^0-9]/g, '');
                          setLocalMarketAcrePrice(numbersOnly);
                        }}
                        placeholder="Enter Local Market Acre Price"
                        InputLabelProps={{ shrink: true }}
                        InputProps={{
                          readOnly: !editable
                        }}
...
export default RecentTransactionForm;
