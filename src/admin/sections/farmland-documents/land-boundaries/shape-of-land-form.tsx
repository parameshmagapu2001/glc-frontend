...
function ShapeOfLandForm({ farmlandId, documentIndex, documentDetails, onNext }: Props) {
  ...
  useEffect(() => {
    getIdentifiers('land_shape');
    setComments(documentDetails.documentDetails?.comments || '');
    setSelectedShape(documentDetails.documentDetails?.label || '0');
  ...
  const onSave = async () => {
    if (!selectedShape || selectedShape === '0') {
      enqueueSnackbar('Please select shape of land', {
        variant: 'error',
    ...
    const body: FarmlandDocumentBody = {
      ...
      documentDetails: {
        comments,
        id: identifiers.find((item) => item.label === selectedShape)?.id,
        label: selectedShape
      },
    };
  ...
  const refreshDocumentData = useCallback(async () => {
    ...
    try {
      ...
      setSelectedShape(res.data.documentDetails?.label || '0');
      setActivityTimeLine(res.data.activityTimeline);
      setTimelineViewStatus(res.data.timeLineView || false);
      setActiveView(res.data.timeLineView ? 'timeline' : 'file');
    } catch (err) {
  ...
  const handleDropdownChange = (event: React.ChangeEvent<{ value: unknown }>) => {
    setSelectedShape(event.target.value as string);
  };
      ...
      {activeView === 'file' &&
        ...
        >
          <Stack direction='row' justifyContent='space-between' width='100%'>
            <Box width="40%">
              <Typography fontWeight="medium" sx={{ mb: 2 }}>Shape of the Land:</Typography>
...
export default ShapeOfLandForm;
