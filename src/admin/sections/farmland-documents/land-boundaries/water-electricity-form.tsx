...
function WaterElectricityForm({ farmlandId, documentIndex, documentDetails, onNext }: Props) {
  ...
  useEffect(() => {
    ...
    setFacilityType(documentDetails.documentDetails?.facilityType || '');
    setWaterFacility(documentDetails.documentDetails?.waterFacility || '');
    setElectricityFacility(documentDetails.documentDetails?.electricityFacility || '');
  ...
  const handleStyleRadioChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setFacilityType((event.target as HTMLInputElement).value);
  };

  const handleWaterFacilityChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setWaterFacility((event.target as HTMLInputElement).value);
  };

  const handleElectricityFacilityChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setElectricityFacility((event.target as HTMLInputElement).value);
  };
  ...
  const onSave = async () => {
    if (!facilityType) {
      enqueueSnackbar('Please select available facility', {
        variant: 'error',
    ...
    if (facilityType === 'waterFacility' && !waterFacility) {
    ...
    if (facilityType === 'electricityFacility' && !electricityFacility) {
      enqueueSnackbar('Please select electricity facility', {
        variant: 'error',
        anchorOrigin: { vertical: 'bottom', horizontal: 'center' }
      });
      return;
  ...
  const refreshDocumentData = useCallback(async () => {
    ...
    try {
      ...
      setFacilityType(res.data.documentDetails?.facilityType || '');
      setWaterFacility(res.data.documentDetails?.waterFacility || '');
      setElectricityFacility(res.data.documentDetails?.electricityFacility || '');
      setActivityTimeLine(res.data.activityTimeline);
      setTimelineViewStatus(res.data.timeLineView || false);
      setActiveView(res.data.timeLineView ? 'timeline' : 'file');
    } catch (err) {
          ...
          <Stack direction='row' justifyContent='space-between' width='100%'>
            ...
            <Box width="40%">
              ...
              <FormProvider {...methods}>
                <Stack sx={{ mb: 1 }}>
                  <Typography fontWeight="medium" sx={{ mb: 2 }}>Select available facility</Typography>

                  <RadioGroup aria-label="text" name="video_style" value={facilityType}
                    onChange={(e) => editable && handleStyleRadioChange(e)}
                  >
                    <FormControlLabel value="waterFacility" control={<Radio />} label="Water Facility" />
                    <FormControlLabel value="electricityFacility" control={<Radio />} label="Electricity Facility" />
                    <FormControlLabel value="Both" control={<Radio />} label="Both" />
                  </RadioGroup>
                </Stack>
                {(facilityType === 'Both' || facilityType === 'waterFacility') &&
                  <Stack sx={{ mb: 1 }}>
                    <Typography fontWeight="medium" sx={{ mb: 2 }}>Select Water Facility</Typography>

                    <RadioGroup aria-label="text" name="video_style" value={waterFacility}
                      onChange={(e) => editable && handleWaterFacilityChange(e)}
                    >
                      <FormControlLabel value="Bore" control={<Radio />} label="Bore" />
                      <FormControlLabel value="Muncipal" control={<Radio />} label="Muncipal" />
                    </RadioGroup>
                  </Stack>}

                {(facilityType === 'Both' || facilityType === 'electricityFacility') &&
                  <Stack sx={{ mb: 1 }}>
                    <Typography fontWeight="medium" sx={{ mb: 2 }}>Select Electricity Facility</Typography>

                    <RadioGroup aria-label="text" name="video_style" value={electricityFacility}
                      onChange={(e) => editable && handleElectricityFacilityChange(e)}>
                      <FormControlLabel value="2Phase" control={<Radio />} label="2 Phase" />
                      <FormControlLabel value="3Phase" control={<Radio />} label="3 Phase" />
                    </RadioGroup>
                  </Stack>}
              </FormProvider>
            </Box>
...
export default WaterElectricityForm;
