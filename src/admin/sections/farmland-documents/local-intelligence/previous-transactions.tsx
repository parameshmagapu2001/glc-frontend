...
import { Box, Button, Dialog, DialogActions, DialogContent, FormControlLabel, Grid, Radio, RadioGroup, Stack, TextareaAutosize, Typography, useTheme } from '@mui/material';
...
function PreviousTransactions({ farmlandId, documentDetails, onNext, documentIndex }: Props) {
  ...
  useEffect(() => {
    ...
    setAgreementType(documentDetails.documentDetails?.agreementType || '');
    setAmount(documentDetails.documentDetails?.amount || '');
  ...
  const handleStyleRadioChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setAgreementType((event.target as HTMLInputElement).value);
  };

  const onSave = async () => {
    if (agreementType === '') {
      enqueueSnackbar('Please select the agreement status', {
        variant: 'error',
    ...
    if (agreementType === 'Available' && amount === '') {
      enqueueSnackbar('Please enter the amount', {
        variant: 'error',
        anchorOrigin: { vertical: 'bottom', horizontal: 'center' }
      });
      return;
  ...
  const refreshDocumentData = useCallback(async () => {
    ...
    try {
      ...
      setAgreementType(res.data.documentDetails?.agreementType || '');
      setAmount(res.data.documentDetails?.amount || '');
      setActivityTimeLine(res.data.activityTimeline);
      setTimelineViewStatus(res.data.timeLineView || false);
      setActiveView(res.data.timeLineView ? 'timeline' : 'file');
    } catch (err) {
  ...
  const goToFarmlands = useCallback(() => {
    if (user?.role_id === 8) {
      router.push(paths.io.allFarmlands);
    } else if (user?.role_id === 6) {
      router.push(paths.vo.all_farmland_list);
    } else if (user?.role_id === 1) {
      router.push(paths.superAdmin.all_farmland_list);
    }
  }, [router, user?.role_id]);
      ...
      {activeView === 'file' &&
        ...
        >
          <Stack direction='row' justifyContent='space-between' alignItems='center'>
            <Box width="40%">
              <Typography fontWeight="medium">Any paper agreement on this land?</Typography>

              <FormProvider {...methods}>
                <Stack sx={{ mb: 1 }}>

                  <RadioGroup aria-label="text" name="video_style" value={agreementType}
                ...
                {agreementType === 'Available' &&
                  <Stack sx={{ my: 2 }} spacing={2}>
                    <Typography fontWeight="medium">Last Price of the land when made agreement?</Typography>
                    <RHFTextField
                      name="amount"
                      value={amount}
                      disabled={!editable}
        ...
        <DialogContent>
          <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', mt: 3 }}>
            <Typography variant="h6" align="center" sx={{ mt: 2 }}>
              {user?.role_id === 8 && 'Farmland Submitted'}
              {(user?.role_id === 6 || user?.role_id === 1) && 'Farmland Approved'}
            </Typography>
            ...
            <Typography variant="body1" align="center" sx={{ mt: 2, p: 1, px: 3 }}>
              {user?.role_id === 8 &&
                'Farmland Document has been successfully submitted.'}
              {(user?.role_id === 6 || user?.role_id === 1) &&
                'Farmland Document has been successfully approved.'}
            </Typography>
          </Box>
        </DialogContent>
        <DialogActions>
