...
function PendingLoansForm({ farmlandId, documentDetails, onNext, documentIndex }: Props) {
  ...
  useEffect(() => {
    ...
    setPendingLoan(documentDetails.documentDetails?.pendingLoan || '');
    setLoanAmount(documentDetails.documentDetails?.loanAmount || '');
  ...
  const handleStyleRadioChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setPendingLoan((event.target as HTMLInputElement).value);
  };

  const onSave = async () => {
    if (!pendingLoan) {
      enqueueSnackbar('Please select the pending loan status', {
        variant: 'error',
    ...
    if (pendingLoan === 'Available' && !loanAmount) {
      enqueueSnackbar('Please enter the loan amount', {
        variant: 'error',
        anchorOrigin: { vertical: 'bottom', horizontal: 'center' }
      });
      return;
      ...
      {activeView === 'file' &&
        ...
        >
          <Stack direction='row' justifyContent="space-between" alignItems="center" sx={{ mb: 2 }}>
            <Box width="40%">
              <Typography fontWeight="medium">Any Bank Loans or Pending Loans on the Land?</Typography>

              <FormProvider {...methods}>
                <Stack sx={{ mb: 1 }}>

                  <RadioGroup aria-label="text" name="video_style" value={pendingLoan}
                ...
                {pendingLoan === 'Available' &&
                  <Stack sx={{ my: 2 }} spacing={2}>
                    <Typography fontWeight="medium">Please Enter Loan Amount</Typography>
                    <RHFTextField
                      name="loanAmount"
                      value={loanAmount}
                      onChange={(e) => {
...
export default PendingLoansForm;
