...
function OwnerMindsetForm({ farmlandId, documentDetails, onNext, documentIndex }: Props) {
  ...
  useEffect(() => {
    ...
    setSelectedMindset(documentDetails.documentDetails?.mindset || '0');
  ...
  const onSave = async () => {
    if (selectedMindset === '0') {
      enqueueSnackbar('Please select owner mindset', {
        variant: 'error',
  ...
  const handleDropdownChange = (event: React.ChangeEvent<{ value: unknown }>) => {
    setSelectedMindset(event.target.value as string);
  };
            ...
            <Box width="40%">
              ...
              <FormProvider {...methods}>
                <Stack sx={{ mb: 1 }} spacing={2}>
                  <Typography fontWeight="medium">Owner Mindset</Typography>

                  <RHFSelect
                    name="ownerMindset"
                    placeholder="Select Owner Mindset"
                  ...
                  >
                    <MenuItem value="0">Select Owner Mindset</MenuItem>
...
export default OwnerMindsetForm;
