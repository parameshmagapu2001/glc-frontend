...
function AnyIssuesForm({ farmlandId, documentDetails, onNext, documentIndex }: Props) {
  ...
  useEffect(() => {
    ...
    setIssues(documentDetails.documentDetails?.issues || '');
  ...
  const handleStyleRadioChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setIssues((event.target as HTMLInputElement).value);
  };

  const onSave = async () => {
    if(!issues) {
      enqueueSnackbar('Please select the issue status', {
        variant: 'error',
        ...
        return (
          <Grid xs={12} md={12} sx={{ display: 'flex', justifyContent: 'flex-end' }}>
            {!editable ? (
              ...
              <>
                {cancelButton &&
                  <Button variant="outlined" size="medium" color="error" sx={{ mr: 2, borderRadius: 10 }} onClick={onCancelEdit}>
                    Cancel
                  </Button>}
        ...
        >
          <Stack direction='row' justifyContent='space-between' alignItems='center'>
            ...
            <Box width="40%">
              ...
              <Typography fontWeight="medium">Any Issues with Boundaries & Owners?</Typography>

              <FormProvider {...methods}>

                <Stack sx={{ mb: 1 }}>

                  <RadioGroup aria-label="text" name="video_style" value={issues}
