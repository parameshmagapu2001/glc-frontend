...
function AgreementsForm({ farmlandId, documentDetails, onNext, documentIndex }: Props) {
  ...
  const [hasEditAccess, setHasEditAccess] = useState(documentDetails.editAccess);
  ...
  useEffect(() => {
    ...
    setAgreement(documentDetails.documentDetails?.agreement || '');
    setAgreementType(documentDetails.documentDetails?.agreementType || '');
    setAmount(documentDetails.documentDetails?.amount || '');
    setFiles(
      Array.isArray(documentDetails.documentDetails?.files)
        ? documentDetails.documentDetails.files
        : []
    );
    setDocuments(
  ...
  const handleAgreementTypeChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setAgreementType((event.target as HTMLInputElement).value);
  };

  const onSave = async () => {
    if (agreement === '') {
      enqueueSnackbar('Please select the agreement type', {
        variant: 'error',
        anchorOrigin: { vertical: 'bottom', horizontal: 'center' },
      });
      return;
    }
    if (agreement === 'Available' && agreementType === '') {
      enqueueSnackbar('Please select the agreement type', {
        variant: 'error',
        anchorOrigin: { vertical: 'bottom', horizontal: 'center' },
      });
    ...
    if (agreement === 'Available' && agreementType ==='Verbal' && amount === '') {
    ...
    if (agreement === 'Available' && agreementType ==='Legal' && files.length === 0) {
      enqueueSnackbar('Please upload the files', {
        variant: 'error',
        anchorOrigin: { vertical: 'bottom', horizontal: 'center' },
      });
      return;
          ...
          <Stack direction="row" justifyContent="space-between" width="100%">
            <Box width="40%">
              ...
              <FormProvider {...methods}>
                ...
                {agreement === 'Available' && (
                  <Stack sx={{ mb: 1 }}>
                    <Typography fontWeight="medium">Agreement Type</Typography>
                    <RadioGroup
                      aria-label="text"
                      name="video_style"
                      value={agreementType}
                      onChange={(e) => editable && handleAgreementTypeChange(e)}
                    >
                ...
                {agreementType === 'Legal' && agreement === 'Available' && (
                  <Stack
                    bgcolor="white"
                    borderRadius="10px"
                    mt="20px"
                    ...
                    <Stack direction="row" justifyContent="space-between" width="100%">
                      <Box>
                        <Typography fontWeight="medium">
                          {editable ? 'Upload Files:' : 'Uploaded Files:'}
                        </Typography>
                        ...
                        {editable && (
                          ...
                          >
                            <Box position="absolute" top="20px" right="20px">
                              <Image
                                src="/assets/images/docUpload.png"
                                width={55}
                                height={55}
                                alt="upload"
                              />
