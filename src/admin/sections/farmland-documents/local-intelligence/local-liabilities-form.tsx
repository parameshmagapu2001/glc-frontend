...
function LocalLiabilitiesForm({ farmlandId, documentDetails, onNext, documentIndex }: Props) {
  ...
  useEffect(() => {
    ...
    setLiabilities(documentDetails.documentDetails?.liabilities || '');
  ...
  const handleStyleRadioChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setLiabilities((event.target as HTMLInputElement).value);
  };

  const onSave = async () => {
    if (!liabilities) {
      enqueueSnackbar('Please select the liabilities status', {
        variant: 'error',
      ...
      {activeView === 'file' &&
        ...
        >
          <Stack direction='row' justifyContent='space-between' alignItems='center'>
            <Box width="40%">
              <Typography fontWeight="medium">Local Liabilities Form</Typography>

              <FormProvider {...methods}>
                <Stack sx={{ mb: 1 }}>

                  <RadioGroup aria-label="text" name="video_style" value={liabilities}
...
export default LocalLiabilitiesForm;
