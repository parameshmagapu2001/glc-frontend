...
import { Box, Button, Grid, MenuItem, Stack, TextareaAutosize, Typography } from '@mui/material';
...
function SourcePersonForm({ farmlandId, documentDetails, onNext, documentIndex }: Props) {

  const [editable, setEditable] = useState<boolean>(documentDetails.documentStatus === 'Pending');

  const [comments, setComments] = useState('');

  const [selectedSourcePerson, setSelectedSourcePerson] = useState('0');
  ...
  useEffect(() => {
    ...
    setContactName(documentDetails.documentDetails?.contactName || '');
    setContactNumber(documentDetails.documentDetails?.contactNumber || '');
    setSelectedSourcePerson(documentDetails.documentDetails?.sourcePerson || '0');
  ...
  const onSave = async () => {
    if (selectedSourcePerson === '0') {
      enqueueSnackbar('Please select source person', {
        variant: 'error',
        anchorOrigin: { vertical: 'bottom', horizontal: 'center' }
      });
      return;
    }
    if (selectedSourcePerson === 'GovernmentPerson' && !contactName) {
      enqueueSnackbar('Please enter contact name', {
        variant: 'error',
        anchorOrigin: { vertical: 'bottom', horizontal: 'center' }
      });
      return;
    }
    if (selectedSourcePerson === 'GovernmentPerson' && !contactNumber) {
      enqueueSnackbar('Please enter contact number', {
        variant: 'error',
        anchorOrigin: { vertical: 'bottom', horizontal: 'center' }
      });
      return;
    ...
    const body: FarmlandDocumentBody = {
      ...
      documentDetails: {
        comments,
        sourcePerson: selectedSourcePerson,
        contactName,
        contactNumber
      },
    };
  ...
  const handleDropdownChange = (event: React.ChangeEvent<{ value: unknown }>) => {
    setSelectedSourcePerson(event.target.value as string);
  };
  ...
  const renderButtons = () => {
    ...
    if (reviewStatus === 'Pending') {
      if (approveAccess) {
        return (
          <Grid xs={12} md={12} sx={{ display: 'flex', justifyContent: 'flex-end' }}>
            <Button variant="outlined" color="inherit" size="medium" sx={{ mr: 2, borderRadius: 10 }}>
              Turnback
            </Button>
              ...
              <FormProvider {...methods}>
                <Stack spacing={2}>
                  <Stack sx={{ mb: 1 }} spacing={2}>
                    <Typography fontWeight="medium">Source Person</Typography>

                    <RHFSelect
                      name="sourcePerson"
                      placeholder="Select Source Person"
                      InputLabelProps={{ shrink: true }}
                      value={selectedSourcePerson}
                      disabled={!editable}
                    ...
                    >
                      <MenuItem value="0">Select Source Person</MenuItem>
                      <MenuItem value="GovernmentPerson">Government Person</MenuItem>
                    </RHFSelect>
                  </Stack>

                  {selectedSourcePerson === 'GovernmentPerson' &&
                    <Stack>
                      <Typography fontWeight="medium">Person Contact Details:</Typography>
                      ...
                      <Stack sx={{ my: 2 }} spacing={2}>
                        ...
                        <RHFTextField
                          ...
                          onChange={(e) => {
                            ...
                            const lettersOnly = input.replace(/[^a-zA-Z\s]/g, ''); // allows letters and spaces
                            setContactName(lettersOnly);
                          }}
                      ...
                      <Stack sx={{ my: 2 }} spacing={2}>
                        <Typography fontWeight="medium">Mobile</Typography>
                        <RHFTextField
                          name="contactNumber"
                          value={contactNumber}
                          disabled={!editable}
                          onChange={(e) => {
                            ...
                            const processedInput = input === '0' ? '' : input.slice(0, 10);
                            setContactNumber(processedInput);
                          }}
                          placeholder="Enter Mobile Number"
                          InputLabelProps={{ shrink: true }}
                          error={!!contactNumber && (contactNumber.length !== 10 || contactNumber.startsWith('0'))}
                          helperText={
                            contactNumber && (contactNumber.length !== 10 || contactNumber.startsWith('0'))
...
export default SourcePersonForm;
