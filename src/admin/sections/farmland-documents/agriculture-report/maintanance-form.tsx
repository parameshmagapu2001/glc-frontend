...
function MaintenanceForm({ farmlandId, documentIndex, documentDetails, onNext }: Props) {
  ...
  const [yieldReturns, setYieldReturns] = useState(0);
  ...
  useEffect(() => {
    getIdentifiers('Crop_Type');
    setComments(documentData.documentDetails?.comments || '');
    setCropsGrown(Array.isArray(documentData.documentDetails?.suggestedCrop)
      ? documentData.documentDetails.suggestedCrop.map((crop) => crop.label)
      : []);
    setYieldReturns(documentData.documentDetails?.bestReturns
      ? parseFloat(documentData.documentDetails.bestReturns) || 0
      : 0);
    ...
    , [documentData, farmlandId, documentDetails.documentId]);
  ...
  const onSave = async () => {
    if (cropsGrown.length === 0) {
      enqueueSnackbar('Please select the crops grown', {
        variant: 'error',
    ...
    if (yieldReturns <= 0) {
      enqueueSnackbar('Please enter the yield returns', {
        variant: 'error',
        anchorOrigin: { vertical: 'bottom', horizontal: 'center' }
      });
      return;
    ...
    const body: FarmlandDocumentBody = {
      ...
      documentDetails: {
        comments,
        suggestedCrop: cropsGrown.map((crop) => ({
          id: identifiers.find((item) => item.label === crop)?.id,
          label: crop,
        })),
        bestReturns: yieldReturns
      },
    };
    console.log(body);
  ...
  const handleDropdownChange = (
    ...
    const mappedCrops = identifiers
      .filter((item) => value.includes(item.label))
      .map((item) => item.label);

    setCropsGrown(mappedCrops);
  };
      ...
      {activeView === 'file' &&
        ...
        >
          <Stack justifyContent='space-between' direction="row" width="100%">
            <Box width="40%">
              <Typography fontWeight="medium" sx={{ mb: 2 }}>If <span style={{ color: '#1D7ABE' }}> Green Land Capital</span> does the maintenance,
                What will be the suggested crop? </Typography>
              ...
              <FormProvider {...methods}>
                ...
                <RHFAutocomplete
                  ...
                  options={identifiers.map((reason) => reason.label)}
                />
