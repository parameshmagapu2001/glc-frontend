...
const generateDepthRanges = (start: number, step: number, count: number) => {
  const ranges: IMapping[] = [];
  let depth = start;

  for (let i = 0; i < count; i += 1) {
    ranges.push({
      id: (i + 1).toString(), // Convert number to string
      label: `${depth} Feets`,
    });
    depth += step;
  }
...
function WaterLevelForm({ farmlandId, documentIndex, documentDetails, onNext }: Props) {
  ...
  useEffect(() => {
    ...
    setSelectedDepth(documentData.documentDetails?.label || '0');
  ...
  const onSave = async () => {
    if (selectedDepth === '0') {
      enqueueSnackbar('Please select depth', {
        variant: 'error',
    ...
    const body: FarmlandDocumentBody = {
      ...
      documentDetails: {
        comments,
        id: identifiers.find((item) => item.label === selectedDepth)?.id,
        label: selectedDepth
      },
    };
  ...
  const handleDropdownChange = (event: React.ChangeEvent<{ value: unknown }>) => {
    setSelectedDepth(event.target.value as string);
  };
      ...
      {activeView === 'file' &&
        ...
        >
          <Stack justifyContent="space-between" direction="row" width="100%">
            ...
            <Box width="40%">
              <Typography fontWeight="medium" sx={{ mb: 2 }}>Depth of Ground Water Level?:</Typography>
              ...
              <FormProvider {...methods}>
                ...
                >
                  <MenuItem value="0">Select Depth</MenuItem>
