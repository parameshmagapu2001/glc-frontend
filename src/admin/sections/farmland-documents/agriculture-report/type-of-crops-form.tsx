...
function TypeOfCropsForm({ farmlandId, documentIndex, documentDetails, onNext }: Props) {
  ...
  useEffect(() => {
    ...
    setSelectedCropType(documentData.documentDetails?.label || '0');
    ...
    , [documentData, documentDetails.documentId, farmlandId]);
  ...
  const onSave = async () => {
    if (selectedCropType === '0') {
      enqueueSnackbar('Please select the type of crops', {
        variant: 'error',
    ...
    const body: FarmlandDocumentBody = {
      ...
      documentDetails: {
        comments,
        id: identifiers.find((item) => item.label === selectedCropType)?.id,
        label: selectedCropType
      },
    };
  ...
  const handleDropdownChange = (event: React.ChangeEvent<{ value: unknown }>) => {
    setSelectedCropType(event.target.value as string);
  };
      ...
      {activeView === 'file' &&
        ...
        >
          <Stack direction="row" justifyContent='space-between' width="100%">
            <Box width="40%">
              <Typography fontWeight="medium" sx={{ mb: 2 }}>Types of Crops available present?:</Typography>
              ...
              <FormProvider {...methods}>
                ...
                >
                  <MenuItem value="0">Select Type of Crops</MenuItem>
...
export default TypeOfCropsForm;
