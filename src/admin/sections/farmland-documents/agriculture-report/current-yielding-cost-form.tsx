...
function CurrentYieldingCostForm({ farmlandId, documentIndex, documentDetails, onNext }: Props) {

  const [editable, setEditable] = useState<boolean>(documentDetails.documentStatus === 'Pending');

  const [yieldingCost, setYieldingCost] = useState(0);
  ...
  useEffect(() => {
    setYieldingCost(Number(documentData.documentDetails?.yieldingCost) || 0);
    setYieldReturns(Number(documentData.documentDetails?.yieldReturns) || 0);
  ...
  const onSave = async () => {
    if (!yieldingCost) {
      enqueueSnackbar('Please enter your yielding cost', {
        variant: 'error',
    ...
    if (!yieldReturns) {
      enqueueSnackbar('Please enter your yield returns', {
        variant: 'error',
        anchorOrigin: { vertical: 'bottom', horizontal: 'center' }
      });
      return;
  ...
  const onApprove = async () => {
    ...
    if (response.data === true) {
      ...
      enqueueSnackbar('Document Approved Failed', {
        variant: 'error',
        anchorOrigin: { vertical: 'bottom', horizontal: 'center' }
      });
    }
          ...
          <Stack direction='row' justifyContent='space-between' width="100%">
            <Box width="40%">
              ...
              <FormProvider {...methods}>
                <Typography fontWeight="medium" sx={{ mb: 2 }}>What is the current yielding cost?</Typography>
...
export default CurrentYieldingCostForm;
