...
function CurrenCultivationTypetForm({ farmlandId, documentIndex, documentDetails, onNext }: Props) {
  ...
  const [cultivationType, setCultivationType] = useState(0);
  ...
  useEffect(() => {
    setName(documentData.documentDetails?.name || '');
    setContactNumber(documentData.documentDetails?.contactNumber || '');
    setCultivationType(Number(documentData.documentDetails?.cultivationType) || 0);
  ...
  const handleStyleRadioChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setCultivationType(Number((event.target as HTMLInputElement).value));
  };
  ...
  const onSave = async () => {
    ...
    if (!contactNumber) {
      enqueueSnackbar('Please enter your contact number', {
        variant: 'error',
        anchorOrigin: { vertical: 'bottom', horizontal: 'center' }
      });
      return;
          ...
          <Stack direction='row' justifyContent='space-between' width="100%">
            <Box width="40%">
              ...
              <FormProvider {...methods}>
                <Typography fontWeight="medium" sx={{ mb: 2 }}>What is the Current Cultivation Type?</Typography>

                <RadioGroup row aria-label="text" name="video_style" value={cultivationType}
                  onChange={(e) => editable && handleStyleRadioChange(e)}>

                  <FormControlLabel value="0" control={<Radio />} label="Self" />
                  <FormControlLabel value="1" control={<Radio />} label="Lease" />
                </RadioGroup>
                ...
                <Stack>
                  ...
                  <RHFTextField
                    ...
                    InputLabelProps={{ shrink: true }}
                    error={contactNumber.length > 0 && contactNumber.length !== 10}
                    helperText={
...
export default CurrenCultivationTypetForm;
