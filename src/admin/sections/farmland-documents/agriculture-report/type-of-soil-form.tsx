...
function TypeOfSoilForm({ farmlandId, documentIndex, documentDetails, onNext }: Props) {

  const [editable, setEditable] = useState<boolean>(documentDetails.documentStatus === 'Pending');

  const [selectedSoilType, setSelectedSoilType] = useState('0');
  ...
  useEffect(() => {
    getIdentifiers('Soil_Type');
    setSelectedSoilType(documentData.documentDetails?.label || '0');
    ...
    , [documentData, documentDetails.timeLineView, farmlandId, documentDetails.documentId]);
  ...
  const onSave = async () => {
    ...
    if (!selectedSoilType || selectedSoilType === '0') {
      enqueueSnackbar('Please select soil type', {
        variant: 'error',
    ...
    const body: FarmlandDocumentBody = {
      ...
      documentDetails: {
        comments,
        id: identifiers.find((item) => item.label === selectedSoilType)?.id,
        label: selectedSoilType
      },
    };
  ...
  const handleDropdownChange = (event: React.ChangeEvent<{ value: unknown }>) => {
    setSelectedSoilType(event.target.value as string);
  };
      ...
      {activeView === 'file' &&
        ...
        >
          <Stack direction='row' justifyContent='space-between' width='100%'>
            <Box width="40%">
              <Typography fontWeight="medium" sx={{ mb: 2 }}>Soil Type:</Typography>
              ...
              <FormProvider {...methods}>
                ...
                >
                  <MenuItem value="0">Select Soil Type</MenuItem>
