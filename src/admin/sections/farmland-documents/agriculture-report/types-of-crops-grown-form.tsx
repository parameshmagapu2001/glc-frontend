...
function TypesOfCropsGrownForm({ farmlandId, documentIndex, documentDetails, onNext }: Props) {
  ...
  useEffect(() => {
    ...
    setCropsGrown(
      Array.isArray(documentData.documentDetails?.cropsGrown)
        ? documentData.documentDetails.cropsGrown.map((crop) => crop.label)
        : []
    );
  ...
  const onSave = async () => {
    ...
    const body: FarmlandDocumentBody = {
      ...
      documentDetails: {
        comments,
        cropsGrown: cropsGrown.map((crop) => ({
          ...
          <Stack direction="row" justifyContent="space-between" width="100%">
            <Box width="40%">
              <Typography fontWeight="medium" sx={{ mb: 2 }}>
                Types of Crop can be Grown in Furure? :
              </Typography>
              ...
              <FormProvider {...methods}>
                <RHFAutocomplete
                  ...
                  readOnly={!editable}
                  sx={{ width: 1 }}
                  options={identifiers.map((item) => item.label)}
                />
...
export default TypesOfCropsGrownForm;
