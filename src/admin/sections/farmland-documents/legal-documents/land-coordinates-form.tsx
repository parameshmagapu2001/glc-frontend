...
function LandCoordinatesForm({ farmlandId, documentIndex, documentDetails, onNext }: Props) {
  ...
  useEffect(() => {
    setLandCoordinates(documentData.documentDetails?.landCoordinates || '');
    ...
    fetchTimeline();
  }, [farmlandId, documentData.documentDetails?.comments,
    documentData.documentDetails?.landCoordinates,
  ...
  const onSave = async () => {
    if (landCoordinates === null || landCoordinates === '') {
      enqueueSnackbar('Please enter land coordinates', {
        variant: 'error',
  ...
  const refreshDocumentData = useCallback(async () => {
    ...
    try {
      ...
      setLandCoordinates(res.data.documentDetails?.landCoordinates || '');
      setActivityTimeLine(res.data.activityTimeline);
      setTimelineViewStatus(res.data.timeLineView || false);
      setActiveView(res.data.timeLineView ? 'timeline' : 'file');
    } catch (err) {
          ...
          <Stack direction='row' justifyContent='space-between' width="100%">
            <Box width="40%">
              ...
              <FormProvider {...methods}>
                ...
                <Typography fontWeight="medium" sx={{ mb: 2 }}>Land Coordinates: </Typography>
                ...
                <RHFTextField
                  ...
                  onChange={(e) => {
                    setLandCoordinates(e.target.value);
                  }}
...
export default LandCoordinatesForm;
