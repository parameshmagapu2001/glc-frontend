...
import { Alert, Box, Button, Grid, Snackbar, Stack, TextareaAutosize, Typography } from '@mui/material';
...
function KycVideoForm({ farmlandId, documentIndex, documentDetails, onNext }: Props) {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    selectedFiles.forEach((file) => {
      ...
      if (!isMp4) {
        setSnackbarMessage(`"${file.name}" is not a valid MP4 file.`);
  ...
  const onSave = async () => {
    if (files.length === 0) {
      enqueueSnackbar('Please upload document', {
        variant: 'error',
                ...
                {files && files?.length > 0
                  ? [...Array.from(files)].map((item, idx) => (
                    ...
                    >
                      ...
                      >
                        <Image
                          src="/assets/icons/field-officer/mp4.svg"
                          width={17}
                          height={17}
                          alt="mp4"
                        />
