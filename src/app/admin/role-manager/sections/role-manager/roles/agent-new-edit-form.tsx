...
import { getFilterRegionAreas, getFilterStateRegions, getFilterStates } from 'src/api/filters';
import { IRoleUserRequest } from 'src/types/role-users';
import { createUser, getAreaOfficerDetails, getOfficerDetails, updateUser, uploadUserIdProofs } from 'src/api/roles';
...
export default function AgentNewEditForm({ currentUser, userId }: Props) {
  ...
  const [areas, setAreas] = useState<IMapping[]>([{ id: '0', label: 'Select A Area' }]);

  const [officerDetails, setOfficerDetails] = useState<any>();

  const [areaOfficerDetails, setAreaOfficerDetails] = useState<any>();

  const [intelligenceOfficer, setIntelligenceOfficer] = useState<any>();
  ...
  const UserSchema = Yup.object().shape({
    ...
    pin_code: Yup.string().required('Pin Code is required').min(6, 'Minimum 6 characters').max(6, 'Maximum 6 characters'),
    aadhar_number: Yup.string()
      .required('Aadhar Number is required')
      .min(12, 'Minimum 12 characters').max(12, 'Maximum 12 characters'),
  ...
  const defaultValues = useMemo(
    () => ({
      role_id: currentUser?.role_id || 10,
  ...
  useEffect(() => {
    if (currentUser) {
      ...
      fetchFilterRegionAreas(currentUser.region_id);
    }
    getStates();
  ...
  const handleRegionChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    fetchFilterRegionAreas(parseInt(regionId, 10));
    fetchOfficerDetails(8, parseInt(regionId, 10));
  }

  const fetchFilterRegionAreas = async (stateId: number) => {
    const response = await getFilterRegionAreas(stateId);
    setAreas([{ id: '0', label: 'Select A Area' }, ...response]);
  };

  const handleAreaChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    const areaId = event.target.value;
    setValue('area_id', parseInt(areaId, 10));
    trigger('area_id', { shouldFocus: false });
    fetchAreaOfficerDetails(9, parseInt(areaId, 10));
  }
  ...
  const fetchOfficerDetails = async (roleId: number, regionId: number) => {
    try {
      ...
      if (roleId === 7) {
        ...
        setIntelligenceOfficer(response.data);
      }
    } catch (error) {
      console.error(error);
    }
  ...
  const fetchAreaOfficerDetails = async (roleId: number, areaId: number) => {
    try {
      const response = await getAreaOfficerDetails(roleId, areaId);
      setAreaOfficerDetails(response.data);
    } catch (error) {
      console.error(error);
    }
  }
      ...
      {!successFlag ?
        <Grid container spacing={2} sx={{ px: 10, py: 1, mb: 0 }}>
          ...
          <Grid xs={12} md={7} sx={{
            ...
            <img
              src="/assets/images/agentForm.png"
              alt="Agent"
              style={{
          ...
          <Grid xs={12} md={5} sx={{
            ...
            <Stack spacing={3}>
              ...
              {(officerDetails && officerDetails !== 'NotFound') || (intelligenceOfficer && intelligenceOfficer !== 'NotFound') ? (
                <Stack direction="row" spacing={2} justifyContent='flex-end'>
                  {officerDetails !== 'NotFound' && <Typography variant="body2" sx={{ color: 'success.main' }}>{officerDetails}</Typography>}
                  {officerDetails && officerDetails !== 'NotFound' && intelligenceOfficer !== 'NotFound' && '|'}
                  {intelligenceOfficer !== 'NotFound' && <Typography variant="body2" sx={{ color: 'success.main' }}>{intelligenceOfficer}</Typography>}
                </Stack>
              ) : null}
              <RHFSelect
                name="area_id"
              ...
              {areaOfficerDetails && areaOfficerDetails !== 'NotFound' &&
                <Stack direction="row" spacing={2} justifyContent="flex-end">
                  <Image src="/assets/images/brakeWarning.png" alt="Arrow" height={10} width={10} />
                  <Typography variant="body2" sx={{ color: 'success.main' }}>{areaOfficerDetails}</Typography>
                </Stack>}

              <Typography variant="h6">Enter Agent Details</Typography>
        ...
        <Stack direction="column" alignItems="center" justifyContent="center" sx={{ width: '100%', height: '65vh' }}>
          <Image
