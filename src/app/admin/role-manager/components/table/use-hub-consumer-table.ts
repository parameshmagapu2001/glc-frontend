...
export type UseTableProps = {
  ...
  hub_id: number;
  fetchData: (hub_id:number, pageNumber: number, pageSize: number) => Promise<void>;
};

export default function useHubConsumerTable(props?: UseTableProps): ReturnType {
  const fetchData = props?.fetchData;
  const current_hub_id = props?.hub_id || 0;
  ...
  const onChangeRowsPerPage = useCallback(async(event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setRowsPerPage(rows);
      if(fetchData) {
        await fetchData(current_hub_id, 0, rows);
      }
    },
    [fetchData, current_hub_id]
  );
  ...
  const onChangePage = useCallback(
    async (event: unknown, newPage: number) => {
      if (fetchData) {
        await fetchData(current_hub_id, newPage, rowsPerPage);
      }
      setPage(newPage);
    },
    [fetchData, rowsPerPage, current_hub_id]
  );
